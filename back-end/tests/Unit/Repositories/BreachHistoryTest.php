...
class BreachHistoryTest extends TestCase
{
    ...
    public function test_check_if_renter_has_same_breach_history() {
        $renter = User::factory()->create(['role' => User::ROLE_RENTER, 'occupation' => 'test data']); //role renter
        $breach = Breach::factory()->create(['description' => 'test data']);
        $this->assertFalse($this->breach_history_repository->checkIfRenterHasSameBreachHistory($renter->id, $breach->id));
        $breach_history = BreachHistory::factory()->create(['renter_id' => $renter->id, 'breach_id' => $breach->id]);
        $this->assertTrue($this->breach_history_repository->checkIfRenterHasSameBreachHistory($renter->id, $breach->id));
    }
